...
- Prefer using built-in tools like `Read`, `Grep`, `Glob`, `Edit` for simple find / read / edit operations over calling bash / shell commands like `cat`, `grep`, `find`, `sed`.
...
However, you SHOULD NOT attempt to run tests like:
...
because one typo in your test call itself could lead to actually executing a dangerous command in your test procedure ironically! You may still try to test out clever bypasses and edge cases, but avoid crafting test cases that are obviously destructive commands that could wreak havoc on the system if there was a typo.
