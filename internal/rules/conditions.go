...
type EvalCtx struct {
	...
	// using the walker's symtab for simple variable expansion. If nil,
	// falls back to check.LiteralString.
	ResolveWord func(word *syntax.Word) (string, bool)
...
func evaluateRefCommandSpec(refName string, parsed *parsedArgs, ctx *EvalCtx) bool {
	...
	case *rulespb.CommandSpec_CustomRules:
		...
		reparsed, ok := parseArgs(parsed.rawWords, collectFlagDefs(c.CustomRules), c.CustomRules.GetSupportCombinedShortFlags(), ctx.ResolveWord)
		if !ok {
			return false
		}
...
func evalRecurseEval(_ string, parsed *parsedArgs, ctx *EvalCtx) bool {
	// Join raw words starting from the first positional onward, skipping any
	// consumed flags that precede it (e.g., wrapper-specific flags like -u).
	words := parsed.rawWords
	if len(parsed.positionalWordIndices) > 0 {
		words = parsed.rawWords[parsed.positionalWordIndices[0]:]
	}
	remaining, ok := check.LiteralArgs(words)
	if !ok {
		return false
	}
