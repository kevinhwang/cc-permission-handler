...
func recurseEvalCheck() *rulespb.Check {
	return &rulespb.Check{Check: &rulespb.Check_RecurseEval{RecurseEval: &rulespb.RecurseEval{}}}
}
...
func TestParserBasics(t *testing.T) {
	t.Run("flags and positionals", func(t *testing.T) {
		...
		parsed, ok := parseArgs(words[1:], flagDefs, false, nil)
		if !ok {
...
func TestParserCombinedShortFlags(t *testing.T) {
	t.Run("basic decomposition", func(t *testing.T) {
		...
		parsed, ok := parseArgs(words[1:], flagDefs, true, nil)
		if !ok {
			t.Fatal("parseArgs failed")
		}
...
func TestRecurseEval(t *testing.T) {
	...
	t.Run("basic recurse", func(t *testing.T) {
		...
		spec := makeSpec([]string{"wrapper"}, []*rulespb.Rule{
			{Action: &rulespb.Rule_AlwaysAllow{AlwaysAllow: &rulespb.AlwaysAllow{}}},
	...
	t.Run("skips consumed flags before first positional", func(t *testing.T) {
		// Wrapper with -u <user> flag: flag+arg should be stripped from
		// the recursive command.
		spec := makeSpec([]string{"sudo-wrapper"}, []*rulespb.Rule{
			{Action: &rulespb.Rule_AlwaysAllow{AlwaysAllow: &rulespb.AlwaysAllow{}}},
			denyRule(notCond(everyPositionalPassesCond(recurseEvalCheck()))),
		})
		// Define -u as consuming an arg so the parser strips it.
		spec.Checker.(*rulespb.CommandSpec_CustomRules).CustomRules.Rules = append(
			spec.Checker.(*rulespb.CommandSpec_CustomRules).CustomRules.Rules,
