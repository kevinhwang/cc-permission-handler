...
func Evaluate(spec *rulespb.CommandSpec, ctx *EvalCtx, args []*syntax.Word) bool {
	...
	case *rulespb.CommandSpec_CustomRules:
		...
		parsed, ok := parseCommand(c.CustomRules, args, ctx.ResolveWord)
		if !ok {
			return false
		}
...
func parseCommand(cr *rulespb.CustomRules, args []*syntax.Word, resolve resolveWordFunc) (*parsedArgs, bool) {
	flagDefs := collectFlagDefs(cr)
	return parseArgs(args[1:], flagDefs, cr.GetSupportCombinedShortFlags(), resolve)
}
