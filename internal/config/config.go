...
// MatchingProjects returns all projects whose path patterns match cwd,
// in config declaration order.
func MatchingProjects(cwd string) []*configpb.Project {
	return matchingProjects(global, cwd)
}

// PreferNativeTools returns true if any matching project for the given cwd
// has prefer_native_tools enabled.
func PreferNativeTools(cwd string) bool {
	return preferNativeTools(global, cwd)
}

func preferNativeTools(c *configpb.Config, cwd string) bool {
	for _, proj := range matchingProjects(c, cwd) {
		if proj.GetPreferNativeTools() {
			return true
		}
	}
	return false
