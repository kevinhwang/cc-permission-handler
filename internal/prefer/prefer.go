...
var preferredTools = map[string]preferredTool{
	...
	"sed":   {"Edit/Update", "Use the Edit or Update tool for file modifications, or Read for viewing."},
	"awk":   {"Edit/Update/Read/Grep", "Use the Edit, Update, Read, or Grep tools instead."},
	"gawk":  {"Edit/Update/Read/Grep", "Use the Edit, Update, Read, or Grep tools instead."},
...
func checkSingleCall(call *syntax.CallExpr) (string, bool) {
	...
	msg := fmt.Sprintf(
		"Avoid using `Bash(%s)` for this — %s\n"+
			"If you genuinely need this Bash command, put the text %s in the `description` (**description** field of `tool_input`, not the command itself) of the Bash tool call.",
