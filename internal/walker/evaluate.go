...
func Evaluate(command, cwd string, ruleSet *rulespb.RuleSet) bool {
	return EvaluateWithDirs(command, cwd, nil, ruleSet)
}
...
func EvaluateWithDirs(command, cwd string, writeDirs []string, ruleSet *rulespb.RuleSet) bool {
	...
	return EvaluateAST(f, cwd, writeDirs, ruleSet)
}

// EvaluateAST evaluates a pre-parsed command AST.
func EvaluateAST(f *syntax.File, cwd string, writeDirs []string, ruleSet *rulespb.RuleSet) bool {
	if len(f.Stmts) == 0 {
		return false
	}
	evalFunc := func(command, cwd string, writeDirs []string) bool {
		return EvaluateWithDirs(command, cwd, writeDirs, ruleSet)
	}
	w := newWalker(cwd, writeDirs, evalFunc, ruleSet)
