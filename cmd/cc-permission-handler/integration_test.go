...
var integrationRuleSet *rulespb.RuleSet

func init() {
	var err error
	integrationRuleSet, err = rules.DefaultRules()
	if err != nil {
		panic(err)
	}
}
...
func runFullHookTest(t *testing.T, command, cwd, description string) (allowed bool, output string) {
	t.Helper()
	return runFullHookTestWithPrefer(t, command, cwd, description, true)
}

func runFullHookTestWithPrefer(t *testing.T, command, cwd, description string, preferNative bool) (allowed bool, output string) {
	t.Helper()
	...
	if preferNative && !prefer.HasSuppressPrefix(parsed.ToolInput.Description) {
	...
	if walker.EvaluateAST(f, parsed.Cwd, nil, integrationRuleSet) {
		writeAllow(&stdout)
...
func TestIntegration_PreferNativeToolsDisabled(t *testing.T) {
	const cwd = "/project"

	t.Run("cat allowed when prefer_native_tools disabled", func(t *testing.T) {
		allowed, _ := runFullHookTestWithPrefer(t, "cat file.txt", cwd, "", false)
		if !allowed {
			t.Error("cat should be allowed when prefer_native_tools is disabled")
		}
	})
	t.Run("grep allowed when prefer_native_tools disabled", func(t *testing.T) {
		allowed, _ := runFullHookTestWithPrefer(t, "grep pattern file", cwd, "", false)
		if !allowed {
			t.Error("grep should be allowed when prefer_native_tools is disabled")
		}
	})
	t.Run("cat denied when prefer_native_tools enabled", func(t *testing.T) {
		allowed, output := runFullHookTestWithPrefer(t, "cat file.txt", cwd, "", true)
		if allowed {
			t.Error("cat should be denied when prefer_native_tools is enabled")
		}
		if output == "" {
			t.Error("should have deny output")
		}
	})
}
