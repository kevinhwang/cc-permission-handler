...
func TestEvaluate_Allow(t *testing.T) {
	tests := []struct {
		...
		{name: "env print null-delimited", cmd: "env -0"},
		{name: "env with vars only", cmd: "env FOO=bar BAZ=qux"},
		...
		{name: "env compound", cmd: "cd dir && env FOO=bar agent-bzl test //pkg:test"},
...
func TestEvaluate_FallThrough(t *testing.T) {
	tests := []struct {
		...
		{name: "git config set value", cmd: `git config user.name "Kevin"`},
		{name: "git config core.hooksPath", cmd: "git config core.hooksPath /tmp/evil"},
		{name: "git config core.pager", cmd: `git config core.pager "rm -rf /"`},
		{name: "git config alias", cmd: `git config alias.x '!rm -rf /'`},
		{name: "git config --global set", cmd: `git config --global user.email "x@y"`},
		{name: "git config --add", cmd: "git config --add user.name Kevin"},
		{name: "git config --unset", cmd: "git config --unset user.name"},
		{name: "go run /tmp", cmd: "go run /tmp/evil.go"},
		...
		{name: "sort -o", cmd: "sort -o output.txt input.txt"},
		{name: "sort --output", cmd: "sort --output=output.txt input.txt"},
		{name: "sort -no", cmd: "sort -no output.txt input.txt"},
...
func TestEvaluate_NoCwd(t *testing.T) {
	t.Run("redirect to dev null", func(t *testing.T) {
		if !Evaluate("git log > /dev/null", "") {
			t.Error("expected allow")
		}
	})
	t.Run("redirect to file falls through", func(t *testing.T) {
		if Evaluate("git log > output.txt", "") {
